					...
					for rowCoordinate, row in rows
						...
						<svg:g>
							...
							for colCoordinate, col in cols
								...
								let color = (verifyEven(row) and verifyEven(col)) or (!verifyEven(row) and !verifyEven(col)) ? "#E6EDDE" : "#618340"
								let index = (row+2)*12 + (col+2)
								let x = col*60

								<svg:g :tap=(do transferIndex(index))>
									...
									if @selected
										...
										else if @moves:defense.includes(index)
											<svg:circle r="15" fill="rgba(0,0,255,0.5)" cx="{x+70}" cy="{y+35}">
									if Square[index]
